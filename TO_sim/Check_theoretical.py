...
def K_R_Lorentzian_theoretical(K,Kc):
    R = lambda k: np.sqrt(1-Kc/k)
    r = np.array([R(x) if x>Kc else 0 for x in K])
    return r
def Make_theoretical_KR(Ks,m,d=1):
    def r_Case1(x,m,d):
    ...
    if m == 0:
        Kc = 2 / (np.pi * scs.cauchy.pdf(0, 0, d))
        def K_R_theoretical(K,Kc):
            R = lambda k: np.sqrt(1-Kc/k)
            r = [R(x) if x>Kc else 0 for x in K]
            return r
        KF = Ks
        RF = K_R_theoretical(Ks,Kc) 
        KB = Ks
        RB = K_R_theoretical(Ks,Kc) 
    else:
        KF =[]
        RF =[]
        KB =[]
        ...
        for K_  in  Ks[::1]:
                ...
                # plt.plot(X,X/K_)
                i+=1
                TEMP_2 = (X/K_)[abs(r_Case2(X,m,d)-X/K_)<1e-4]
                if len(TEMP_2)!=0:
                    for R_ in TEMP_2:
                        KB.append(K_)
                        RB.append(R_)
                TEMP_1 = (X/K_)[abs(r_Case1(X,m,d)-X/K_)<1e-4]
                if len(TEMP_1)!=0:
                    for R_ in TEMP_1:
                        KF.append(K_)
                        RF.append(R_)
